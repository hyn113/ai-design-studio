let STEP_COUNT = 64;
const STORAGE_KEY = 'soundi-v4-archive';
...
function getMoodPhrases(analysis) {
  if (!composition.length) {
    return [currentLanguage === 'ko'
      ? '노트를 몇 개 놓으면, 이 음악이 어떻게 느껴지는지 설명해줄게요.'
      : 'Add a few notes and I will describe how this music feels.'];
  }
  if (currentLanguage === 'ko') {
    const rhythmText = analysis.averageGap <= 2
      ? '노트가 빠르게 움직여서 생동감 있는 흐름처럼 보여요.'
      : analysis.averageGap >= 5
        ? '각 노트 사이에 여백이 있어서 차분하게 숨 쉬는 느낌이에요.'
        : '간격이 안정적이라 단순한 문장이 만들어지는 것처럼 보여요.';
    const rangeText = analysis.highRatio > analysis.lowRatio
      ? '멜로디가 높은 쪽에 있어서 더 가볍고 열려 있는 느낌이에요.'
      : analysis.lowRatio > analysis.highRatio
        ? '멜로디가 낮은 쪽에 있어서 더 깊고 안정적으로 느껴져요.'
        : '높은 음과 낮은 음이 균형을 이루어서 분위기가 안정적이에요.';
    if (analysis.paletteKey === 'bright') {
      return [
        '이 음악은 밝고 깨어 있는 느낌이에요. 경쾌한 장면이 시작되는 것 같아요.',
        rhythmText,
        rangeText
      ];
    }
    if (analysis.paletteKey === 'moon') {
      return [
        '이 음악은 차분하고 푸른 느낌이에요. 조용한 밤이나 느린 기억처럼 보여요.',
        rhythmText,
        '긴 음들이 분위기를 부드럽게 이어줘요.'
      ];
    }
    if (analysis.paletteKey === 'sparse') {
      return [
        '이 음악은 미니멀하고 넓은 느낌이에요. 빈 공간에서 작은 멜로디가 흐르는 것 같아요.',
        rhythmText,
        '비어 있는 간격도 음악의 느낌이 돼요.'
      ];
    }
    return [
      '이 음악은 따뜻하고 균형 잡힌 느낌이에요. 이야기를 찾아가는 부드러운 멜로디 같아요.',
      rhythmText,
      rangeText
    ];
  }

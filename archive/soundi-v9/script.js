...
async function unpublishMySong(id, forEdit = false) {
  ...
  const confirmed = await showSongDialog(forEdit ? (currentLanguage === 'en' ? 'Unpublish this song to edit it? You can publish it again after editing.' : '수정하려면 먼저 게시를 취소해요. 수정 후 다시 게시할 수 있어요. 계속할까요?') : (currentLanguage === 'en' ? 'Unpublish this song? It will stay in My Songs.' : '게시를 취소할까요? Archive에서는 내려가지만 저장된 곡은 유지돼요.'), { confirm: true, title: forEdit ? (currentLanguage === 'en' ? 'Edit song' : '곡 수정') : (currentLanguage === 'en' ? 'Unpublish song' : '게시 취소'), actionLabel: forEdit ? (currentLanguage === 'en' ? 'Edit' : '수정하기') : (currentLanguage === 'en' ? 'Unpublish' : '게시 취소') });
  if (!confirmed) return;
...
function openArchiveSongMenu(id, trigger) {
  ...
  if (owner) entries.push(['edit', en ? 'Edit' : '수정'], ['duplicate', en ? 'Create a copy' : '복제해서 만들기'], ['unpublish', en ? 'Unpublish' : '게시 취소'], ['delete', en ? 'Delete' : '삭제']);
...
function showSongDialog(message, { confirm = false, title, actionLabel } = {}) {
  return new Promise(resolve => {
    ...
    dialog.innerHTML = `<h2>${escapeHtml(title || (en ? 'Notice' : '안내'))}</h2><p>${escapeHtml(message)}</p><div class="delete-confirm-actions">${confirm ? `<button type="button" class="delete-confirm-cancel" data-cancel>${en ? 'Cancel' : '취소'}</button>` : ''}<button type="button" class="song-action-accept" data-accept>${escapeHtml(actionLabel || (en ? 'OK' : '확인'))}</button></div>`;
    let accepted = false;
...
function attachDialogCursor(dialog) {
  ...
  dialog.addEventListener('close', () => {
    ...
    if (customCursor.parentElement === dialog) {
      const next = [...document.querySelectorAll('dialog[open]')].at(-1);
